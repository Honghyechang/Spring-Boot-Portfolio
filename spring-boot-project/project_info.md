...
#### 1) 기본 화면 (article-list.html)

![기본 화면](images/article-list-default.png)
![로그인 후 기본 화면](images/article-list-loginAfter-default.png)
![로그인 후 관리 클릭](images/article-list-loginAfter-default-adminClick.png)
![로그인 후 사용자 클릭](images/article-list-loginAfter-userClick.png)

**화면 설명**

`/` 또는 `/article/list`를 요청하면 나타나는 디폴트 화면입니다. 화면은 크게 헤더 영역과 콘텐츠 영역으로 구분됩니다.

**헤더 영역 (네비게이션바)**

네비게이션바는 현재 사용자의 로그인 상태에 따라 다른 메뉴를 표시합니다:

- **비로그인 상태**: "로그인", "회원가입" 메뉴 표시
- **로그인 상태**: 사용자 이름 드롭다운 메뉴 표시
  - 클릭 시 "비밀번호 변경", "로그아웃" 옵션 제공
- **관리자 권한**: "관리" 드롭다운 메뉴 추가 표시
  - 클릭 시 "회원 관리" 옵션 제공

**콘텐츠 영역**

- 게시글 목록을 테이블 형태로 표시 (#, 제목, 작성자, 수정날짜)
- 페이지네이션 바로 페이지 이동 가능
...
![로그인 화면](images/login-page.png)

**화면 설명**

이메일(아이디)과 비밀번호를 입력하여 로그인합니다. 로그인에 성공하면 디폴트 화면(`/article/list`)으로 이동합니다.

**로그인 실패 시**

![로그인 실패](images/login-error.png)

잘못된 이메일 또는 비밀번호를 입력하면 "아이디와 패스워드가 잘못 되었습니다." 오류 메시지가 빨간색으로 표시됩니다.

---

#### 3) 로그아웃 확인 화면

![로그아웃 화면](images/logout-page.png)

**화면 설명**

"로그아웃 하시겠습니까?" 확인 메시지와 함께 로그아웃 버튼이 표시됩니다. 버튼을 클릭하면 세션이 무효화되고 디폴트 화면으로 이동합니다.

---
#### 4) 회원가입 화면

![회원가입 화면](images/signup-page.png)
![회원가입 실패 화면](images/signup-error.png)

**화면 설명**

 회원가입을 하는 페이지로, 아이디가 존재하는지, 적절하게 입력했는지 검사합니다.
 
---

#### 5) 비밀번호 변경 화면

![비밀번호 변경 화면](images/password.png)
![비밀번호 변경 실패 화면](images/password-error.png)

**화면 설명**

 로그인한 사용자가 비밀번호를 변경하는 페이지입니다.
 
---

#### 6) 게시글 상세 화면 (article-content.html)

![게시글 상세](images/article-content-other.png)
![게시글 상세](images/article-content-self.png)

**화면 설명**

게시글 목록에서 제목을 클릭하면 해당 게시글의 상세 내용을 볼 수 있습니다.

**버튼 표시 규칙**

- **목록 버튼**: 모든 사용자에게 표시
- **수정/삭제 버튼**: 게시글 작성자 본인에게만 표시
  - 로그인하지 않았거나 다른 사용자가 작성한 게시글에는 표시되지 않음

---

#### 7) 게시글 수정 화면 (article-edit.html)

![게시글 수정](images/article-edit.png)

**화면 설명**

게시글 상세 화면에서 "수정" 버튼을 클릭하면 나타나는 화면입니다. 기존 제목과 내용이 입력 필드에 미리 채워져 있어, 사용자는 원하는 부분만 수정할 수 있습니다.

---

#### 8) 게시글 작성 화면 (article-edit.html)

![게시글 수정](images/article-add.png)

**화면 설명**

기본 화면에서 로그인한 상태에서 글쓰기 버튼을 클릭 시 나타나는 화면입니다.

---

#### 9) 관리자 페이지 (member-list.html)

![회원 목록](images/member-list.png)

**화면 설명**

관리자 권한(`ROLE_ADMIN`)을 가진 사용자만 접근할 수 있는 회원 관리 화면입니다. 

**표시 정보**

- 회원의 ID, 이름, 이메일 정보를 테이블로 표시
- 각 회원마다 "수정", "삭제" 버튼 제공
- 페이지네이션 기능으로 회원 목록 탐색 가능

---

#### 10) 회원 정보 수정 화면 (member-edit.html)

![회원 수정](images/member-edit.png)

**화면 설명**

관리자가 회원 목록에서 "수정" 버튼을 클릭하면 나타나는 화면입니다.

**수정 가능 항목**

- **이름**: 수정 가능 (입력 필드 활성화)
- **이메일**: 읽기 전용 (로그인 아이디이므로 변경 불가)

기존 회원 정보가 폼에 미리 채워져 있으며, 이름을 수정한 후 "수정" 버튼을 클릭하면 변경 사항이 저장됩니다.
...
**4. 트러블슈팅 능력 향상**
- JavaScript 라이브러리 의존성 문제 해결
- Java 8 Time API와 Thymeleaf 통합
- SpEL 표현식의 타입 안전성 이해
- 데이터베이스 외래키 제약 조건 처리

이러한 경험을 바탕으로 실무 환경에서 요구되는 복합적인 웹 애플리케이션을 설계하고 구현할 수 있는 기반을 마련했습니다.
