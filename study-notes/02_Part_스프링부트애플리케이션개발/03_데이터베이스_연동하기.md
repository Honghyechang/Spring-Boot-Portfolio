...
**MongoDB**는 대표적인 **NoSQL 데이터베이스**로, 관계형 데이터베이스(RDBMS)와 달리 **스키마가 고정되어 있지 않은** 유연한 데이터베이스입니다.

#### RDBMS vs NoSQL (MongoDB) 용어 비교

| 개념 | RDBMS | NoSQL (MongoDB) |
|-----|-------|----------------|
| **저장 단위** | 테이블 (Table) | **컬렉션 (Collection)** |
| **데이터 단위** | 레코드/행 (Record/Row) | **도큐먼트 (Document)** |
| **데이터 속성** | 컬럼 (Column) | **필드 (Field)** |

#### 어노테이션 비교

| 역할 | Spring Data JPA | Spring Data MongoDB |
|-----|----------------|---------------------|
| 클래스 매핑 | `@Entity` | **`@Document`** |
| 기본 키 | `@Id` | **`@Id`** |
| 필드 매핑 | `@Column` | **`@Field`** |
...
MongoDB는 RDBMS에서 구현하기 어려운 다음 두 가지 기능에서 강력한 성능을 발휘합니다:

#### 1. 텍스트 검색 (Full-Text Search) ⭐
...
- 검색어 일치도에 따른 점수 계산
- 점수 기반 정렬
- 복합 검색어 지원 (OR, AND, NOT)

예시: "제주도", "바다", "여행" 중 하나라도 포함된 게시글 검색
```
...
**사용 버전**:
- MongoDB Community Server 8.2.1 (Windows 64-bit)
...
**핵심 의존성**:
- `spring-boot-starter-data-mongodb`: MongoDB 연동을 위한 Spring Data MongoDB
...
```properties
spring.application.name=mongodbSample

# MongoDB 연결 설정 (권장 방식)
spring.data.mongodb.host=localhost
spring.data.mongodb.port=27017
spring.data.mongodb.database=lecture
...
| 속성 | 값 | 역할 |
|-----|-----|------|
| **`host`** | `localhost` | MongoDB 서버 주소 (로컬 호스트) |
| **`port`** | `27017` | MongoDB 기본 포트 |
| **`database`** | `lecture` | 사용할 데이터베이스 이름 |
| **`auto-index-creation`** | `true` | `@Indexed`, `@TextIndexed` 등의 인덱스 자동 생성 |

#### ⚠️ 설정 방식 비교

**잘못된 방식 (URL 방식 - 비권장)**:
```properties
# ❌ 환경 변수나 외부 설정에 의해 덮어써질 수 있음
spring.data.mongodb.url=mongodb://localhost:27017/lecture
```

**문제점**:
- 환경 변수(`SPRING_DATA_MONGODB_DATABASE=test` 등)에 의해 설정이 무시될 수 있음
- `lecture` DB를 지정했는데 기본 DB인 `test`로 연결되는 문제 발생

**권장 방식 (개별 속성)**:
```properties
# ✅ 명확한 우선순위, 외부 설정의 영향을 차단
spring.data.mongodb.host=localhost
spring.data.mongodb.port=27017
spring.data.mongodb.database=lecture
```
...
-- ❌ age 컬럼 없이 INSERT 불가능
INSERT INTO member (id, name, email) VALUES (1, '홍혜창', 'test@example.com');
```

**MongoDB**:
```json
// ✅ 도큐먼트마다 다른 필드를 가질 수 있음
{ "_id": 1, "name": "홍혜창", "email": "test@example.com" }
{ "_id": 2, "name": "김우현", "email": "kim@example.com", "age": 25 }
{ "_id": 3, "title": "제목", "description": "내용" }
```

**핵심**: MongoDB는 **최대 스키마**를 정의하는 방식입니다. 클래스에 정의된 필드가 있어도, 실제 저장 시 해당 필드를 생략할 수 있습니다.
...
[Member 테이블]
id | name    | email
1  | 홍혜창   | hyechang@spring.co.kr

[Article 테이블]
id | member_id | title     | description
1  | 1         | 방학 첫날 | 오늘은 열심히...

→ JOIN 필요: Article 조회 시 Member 정보를 얻으려면 테이블 결합 필요
```
...
// ✅ 하나의 도큐먼트에 모든 정보 포함
{
  "_id": 1,
  "name": "홍혜창",
  "email": "hyechang@spring.co.kr",
  "title": "방학 첫날",
  "description": "오늘은 열심히..."
}

→ JOIN 불필요: 한 번의 조회로 모든 정보 획득 (빠른 읽기 성능)
```

**장점**:
- ✅ JOIN 불필요 → **빠른 읽기 성능**
- ✅ 네트워크 왕복 횟수 감소
- ✅ 데이터 일관성 관리 간단 (하나의 도큐먼트 안에 모든 정보)

**단점**:
- ❌ 데이터 중복 (같은 작성자가 여러 게시글 작성 시 이름/이메일 중복 저장)
- ❌ 업데이트 시 여러 도큐먼트 수정 필요

---

### 3.5.5 Spring Data JDBC/JPA vs Spring Data MongoDB

| 기능 | Spring Data JPA | Spring Data MongoDB |
|-----|----------------|---------------------|
| **DBMS 대상** | RDBMS (MySQL, PostgreSQL 등) | NoSQL (MongoDB) |
| **데이터 구조** | 테이블 → 레코드 → 컬럼 | 컬렉션 → 도큐먼트 → 필드 |
| **스키마** | 고정 (엄격) | 유연 (Schemaless) |
| **기본 CRUD** | `JpaRepository` 상속 | **`MongoRepository`** 상속 |
| **쿼리 메서드** | ✅ 지원 | ✅ **지원** (동일 규칙) |
| **`@Query`** | ✅ JPQL | ✅ **JSON 쿼리** |
| **연관관계** | `@OneToMany`, `@ManyToOne` 등 | 비정규화 권장 (불필요) |

**핵심**: Spring Data MongoDB는 Spring Data JPA와 **사용법이 매우 유사**하지만, MongoDB의 특성(유연한 스키마, 비정규화)을 반영합니다.

---

## 3.5.6 텍스트 검색 (Text Search)

### 3.5.6.1 Article 엔티티

**Article.java**:
```java
package com.example.mongodbSample;
...
**역할**: 이 클래스가 MongoDB의 **컬렉션(Collection)**과 매핑되는 **도큐먼트(Document)**임을 선언

**매핑 규칙**:
- 클래스 이름 → 컬렉션 이름 (소문자)
- `Article` 클래스 → `article` 컬렉션

**명시적 지정**:
```java
@Document(collection = "posts")  // 컬렉션 이름이 규칙과 다를 때
public class Article { ... }
```
...
| 항목 | JPA | MongoDB |
|-----|-----|---------|
| 매핑 선언 | `@Entity` | **`@Document`** |
| 저장 단위 | 테이블 (Table) | 컬렉션 (Collection) |
| 데이터 단위 | 레코드 (Row) | 도큐먼트 (Document) |
...
**역할**: 도큐먼트의 **고유 식별자** (`_id` 필드)

**타입별 동작**:

| Java 타입 | MongoDB 동작 |
|----------|-------------|
| **`String`** | MongoDB가 고유한 `ObjectId` 문자열 자동 생성 (권장) |
| **`ObjectId`** | MongoDB의 네이티브 타입 직접 사용 |
| **`Long`, `Integer`** | 개발자가 직접 ID 값 설정 필요 |

**JPA와 차이점**:

| 항목 | JPA | MongoDB |
|-----|-----|---------|
| 기본 키 타입 | `Long` (숫자) 권장 | **`String`** 권장 |
| 자동 생성 | `@GeneratedValue` 필요 | `String` 타입이면 자동 생성 |
...
**역할**: Java 필드를 MongoDB 도큐먼트의 필드에 매핑

**매핑 규칙**:
- `@Field` 생략 시: Java 필드명 = MongoDB 필드명
- `@Field("email")`: Java 필드명(`email`) → MongoDB 필드명(`email`)
...
|-----|-----|---------|
| 필드 매핑 | `@Column` | **`@Field`** |
| 생략 시 | 필드명 = 컬럼명 | 필드명 = 필드명 |

---

#### @TextIndexed (텍스트 검색의 핵심!) ⭐
...
**역할**: 해당 필드를 **텍스트 검색 인덱스**에 포함시킴

**동작**:
```
1. MongoDB가 이 필드들의 텍스트 인덱스 생성
2. 텍스트 검색 시 이 필드들만 검색 대상이 됨
3. @TextIndexed가 없는 필드는 텍스트 검색 대상 아님
```
...
| 속성 | 의미 |
|-----|------|
| **`weight = 2`** | 이 필드에서 검색어 일치 시 **점수를 2배** 부여 |
| **`weight` 생략** | 기본 가중치 1 |
...
도큐먼트 1:
- title: "제주도 여행" (weight=2) → 점수 2점
- description: "즐거운 여행" → 점수 0점
- 총점: 2점

도큐먼트 2:
- title: "서울 여행" → 점수 0점
- description: "제주도에서 보낸 시간" (weight=1) → 점수 1점
- 총점: 1점

결과: 도큐먼트 1이 먼저 검색됨 (title이 더 중요하다고 판단)
```

**활용**:
- 제목(`title`)을 본문(`description`)보다 중요하게 취급
- 검색 결과의 우선순위 조정
...
**역할**: 텍스트 검색 시 계산된 **검색 적합도 점수**를 저장하는 필드

**동작**:
```
1. TextCriteria로 검색 실행
2. MongoDB가 각 도큐먼트의 일치도 계산
3. 계산된 점수를 @TextScore 필드에 자동 저장
4. 이 점수로 정렬 가능 (높은 점수 = 더 관련성 높음)
```

**점수 계산 요소**:
- 검색어 등장 횟수
- 필드의 가중치(`weight`)
- 검색어의 희소성 (드문 단어일수록 높은 점수)

**JPA와 비교**:

| 항목 | JPA | MongoDB |
|-----|-----|---------|
| 텍스트 검색 | 제한적 (`LIKE` 연산자) | **강력한 전문 검색** |
| 검색 점수 | ❌ 지원 안 함 | ✅ `@TextScore`로 자동 계산 |
| 가중치 | ❌ 지원 안 함 | ✅ `@TextIndexed(weight=...)` |
...
| 어노테이션 | 역할 | JPA 유사 기능 |
|-----------|------|-------------|
| **`@Document`** | 컬렉션 매핑 | `@Entity` (테이블 매핑) |
| **`@Id`** | 고유 식별자 | `@Id` |
| **`@Field`** | 필드 매핑 | `@Column` |
| **`@TextIndexed`** | 텍스트 검색 대상 지정 + 가중치 | ❌ 유사 기능 없음 (LIKE만 가능) |
| **`@TextScore`** | 검색 적합도 점수 저장 | ❌ 유사 기능 없음 |

---

### 3.5.6.4 ArticleRepository
...
public interface ArticleRepository extends MongoRepository<Article, String> {
    ...
    // ========================================
    // 1. 쿼리 메서드 (JPA와 동일한 규칙)
    // ========================================
    List<Article> findByName(String name);
    List<Article> findByTitleContaining(String title);
    List<Article> findByDescriptionStartingWith(String description);
    
    // ========================================
    // 2. 텍스트 검색 (TextCriteria)
    // ========================================
...
**역할**: Spring Data MongoDB의 기본 CRUD 메서드를 자동으로 제공

**제공 메서드**:

| 메서드 | 역할 | MongoDB 명령 |
|-------|------|-------------|
| `save(entity)` | 저장 또는 업데이트 | `insertOne` 또는 `updateOne` |
| `findById(id)` | ID로 조회 | `findOne` |
| `findAll()` | 전체 조회 | `find` |
| `count()` | 전체 개수 | `countDocuments` |
| `deleteById(id)` | ID로 삭제 | `deleteOne` |
| `delete(entity)` | 엔티티 삭제 | `deleteOne` |

**JPA와 비교**:

| 항목 | JPA | MongoDB |
|-----|-----|---------|
| 기본 Repository | `JpaRepository<T, ID>` | **`MongoRepository<T, ID>`** |
| 제공 메서드 | 동일 | **동일** |
| 사용법 | 동일 | **동일** |
...
**Spring Data의 공통 규칙**:
- 메서드 이름만 정의하면 자동으로 쿼리 생성
- JPA와 MongoDB 모두 **동일한 키워드** 사용

| 메서드 패턴 | JPA (SQL) | MongoDB (JSON) |
|-----------|----------|---------------|
| `findByName(String name)` | `WHERE name = ?` | `{ name: ? }` |
| `findByTitleContaining(String title)` | `WHERE title LIKE '%?%'` | `{ title: /.*query.*/ }` (정규표현식) |
| `findByDescriptionStartingWith(String desc)` | `WHERE description LIKE '?%'` | `{ description: /^query/ }` |

**핵심**: 키워드만 알면 JPA와 MongoDB 모두 동일하게 사용 가능!
...
**역할**: `TextCriteria` 객체로 텍스트 검색 실행

| 메서드 | 역할 |
|-------|------|
| **`findBy(TextCriteria, Sort)`** | 검색 + 정렬 |
| **`findBy(TextCriteria, Pageable)`** | 검색 + 페이징 + 정렬 |
| **`findByOrderByScoreDesc(TextCriteria)`** | 검색 + **점수 내림차순 정렬** (가장 많이 사용!) |

**동작 과정**:
```
1. TextCriteria에 검색어 설정 (예: "제주", "바다")
2. MongoDB가 @TextIndexed 필드에서 검색
3. 일치도 점수 계산 → @TextScore 필드에 저장
4. 점수 기준으로 정렬하여 반환
```

---

#### @Query와 @Update (JSON 쿼리) ⭐
...
- `?0`: 첫 번째 매개변수 (`name`)
- `?1`: 두 번째 매개변수 (`email`)
- MongoDB 쿼리: `db.article.find({ name: "홍혜창", email: "test@example.com" })`
...
**동작**:
- `@Query`: 업데이트할 도큐먼트 **찾기** (필터)
- `@Update`: 찾은 도큐먼트 **변경** (명령)
- MongoDB 명령: `db.article.updateOne({ name: "홍혜창" }, { $set: { email: "new@example.com" } })`

---

#### JPA @Query vs MongoDB @Query 비교

| 항목 | JPA | MongoDB |
|-----|-----|---------|
| **쿼리 언어** | JPQL (객체 지향) | **JSON/BSON** (문서 지향) |
| **파라미터 방식** | `:name` (이름 기반, 권장) | **`?0`, `?1`** (위치 기반) |
| **UPDATE 방식** | JPQL 쿼리 전체 작성 | `@Query` (필터) + `@Update` (명령) 분리 |

**예시 비교**:

**JPA**:
```java
@Query("UPDATE Member m SET m.email = :email WHERE m.name = :name")
int updateEmail(@Param("email") String email, @Param("name") String name);
```
...
### 3.5.6.6 TextCriteria 클래스

#### TextCriteria란?

**TextCriteria**는 Spring Data MongoDB에서 제공하는 클래스로, **텍스트 검색 조건**을 정의하는 데 사용됩니다.

**위치**: `org.springframework.data.mongodb.core.query.TextCriteria`

**역할**:
```
1. 검색어 설정
2. 검색 대상: @TextIndexed 필드만
3. 점수 계산: @TextScore 필드에 자동 저장
```
...
TextCriteria criteria = TextCriteria
    .forDefaultLanguage()    // 언어 설정
    .matchingAny("제주", "바다");  // 검색어 설정
```
...
| 메서드 | 역할 | 예시 |
|-------|------|------|
| **`forDefaultLanguage()`** | 서버의 기본 언어 설정 사용 | 한국어 형태소 분석 등 |
| **`matchingAny(...)`** | OR 조건 (하나라도 일치) | `matchingAny("제주", "바다")` |
| **`matching(...)`** | AND 조건 (모두 일치) | `matching("제주 바다")` |
| **`matchingPhrase(...)`** | 구문 일치 (정확한 순서) | `matchingPhrase("제주도 여행")` |
| **`notMatching(...)`** | 제외 조건 | `notMatching("서울")` |
...
1. TextCriteria 생성
   ...
   ↓
2. Repository 메서드 호출
   List<Article> results = articleRepository.findByOrderByScoreDesc(criteria);
   ↓
3. MongoDB가 @TextIndexed 필드에서 검색
   - title 필드 검색 (weight=2)
   - description 필드 검색 (weight=1)
   ↓
4. 일치도 점수 계산
   - "제주"가 title에 있으면: 2점 × 가중치
   - "바다"가 description에 있으면: 1점 × 가중치
   ↓
5. @TextScore 필드에 점수 저장
   article.setScore(3.5);
   ↓
6. 점수 내림차순 정렬하여 반환
   [점수 5.0, 점수 3.5, 점수 2.0, ...]
```

---

### 3.5.6.7 텍스트 검색 실행 예시
...
```java
        // ========================================
        // 1. 텍스트 검색 (TextCriteria)
        // ========================================
        TextCriteria criteria = TextCriteria
            .forDefaultLanguage()
            .matchingAny("제주", "성산일출봉", "바다");
        
        List<Article> articles = articleRepository.findByOrderByScoreDesc(criteria);
        
        log.info("검색 결과: {}개", articles.size());
        ...
        // 2. 업데이트 쿼리
        // ========================================
        int result = articleRepository.updateEmailByName("윤서준", "newEmail@campus.co.kr");
        log.info("업데이트된 도큐먼트 수: {}", result);
        
        // ========================================
        // 3. 업데이트 확인
        // ========================================
        List<Article> updated = articleRepository.findByName("윤서준");
        log.info("업데이트 확인: {}", updated);
    }
}
```
...
**1단계: 텍스트 검색**
```
검색어: "제주", "성산일출봉", "바다" (OR 조건)

검색 대상:
- @TextIndexed가 붙은 title, description 필드만

결과 예시:
게시글 1:
- title: "제주도 여행" (weight=2) → 점수 높음
- description: "아름다운 바다"
- score: 5.2

게시글 2:
- title: "서울 여행"
- description: "제주도에서 본 성산일출봉" (weight=1)
- score: 3.1

정렬: score 내림차순 (5.2 → 3.1 → ...)
```

**2단계: 업데이트**
```
조건: name이 "윤서준"인 도큐먼트
변경: email을 "newEmail@campus.co.kr"로 설정

MongoDB 명령:
db.article.updateOne(
    { name: "윤서준" },
    { $set: { email: "newEmail@campus.co.kr" } }
)
...
Article(
    id=..., 
    name=윤서준, 
    email=newEmail@campus.co.kr,  // ← 변경됨!
    title=..., 
    description=...
)
```
...
### 3.5.6.8 텍스트 검색 핵심 정리

#### MongoDB 텍스트 검색의 3요소

| 요소 | 어노테이션 | 역할 |
|-----|-----------|------|
| **검색 대상 지정** | `@TextIndexed` | 이 필드를 텍스트 인덱스에 포함 |
| **가중치 설정** | `@TextIndexed(weight=2)` | 검색 시 이 필드를 더 중요하게 취급 |
| **점수 저장** | `@TextScore` | 검색 적합도 점수 자동 기록 |
...
```java
// 1. OR 조건 (하나라도 일치)
TextCriteria.forDefaultLanguage().matchingAny("제주", "바다");
// "제주" 또는 "바다"가 포함된 도큐먼트

// 2. AND 조건 (모두 일치)
TextCriteria.forDefaultLanguage().matching("제주 바다");
// "제주"와 "바다"가 모두 포함된 도큐먼트

// 3. 구문 일치 (정확한 순서)
TextCriteria.forDefaultLanguage().matchingPhrase("제주도 여행");
// "제주도 여행"이라는 구문이 정확히 포함된 도큐먼트
...
#### Repository 메서드 선택 가이드

| 상황 | 메서드 |
|-----|--------|
| **검색 + 점수 정렬** | `findByOrderByScoreDesc(criteria)` (가장 많이 사용!) |
| **검색 + 커스텀 정렬** | `findBy(criteria, Sort.by(...))` |
| **검색 + 페이징** | `findBy(criteria, PageRequest.of(...))` |

---

## 3.5.7 공간 검색 (Geospatial Query)

### 3.5.7.1 Pharmacy 엔티티
...
### 3.5.7.2 핵심 어노테이션: @GeoSpatialIndexed
...
**역할**: 이 필드를 **공간 검색 대상**으로 지정

**@TextIndexed와 비교**:

| 어노테이션 | 검색 대상 | 검색 방식 |
|-----------|---------|----------|
| **`@TextIndexed`** | 텍스트 필드 (title, description) | 키워드 검색 (단어 일치) |
| **`@GeoSpatialIndexed`** | 지리 정보 필드 (Point, Polygon) | 거리/반경/영역 검색 |

---

#### GeoSpatialIndexType.GEO_2DSPHERE

**2DSPHERE의 의미**: "2D 구(Sphere) 인덱스"

**역할**: 지구의 **곡률(Curvature)**을 반영하여 거리 계산
...
| 거리 계산 방식 | 오차 |
|-------------|------|
| 평면 공식 (유클리드) | 장거리일수록 오차 큼 (수십~수백 km) |
| **구형 공식 (2DSPHERE)** | **정확한 실제 거리** ✅ |
...
**Point**: Spring Data Geo에서 제공하는 **위도/경도 좌표** 표현 클래스
...
| 순서 | 값 | 범위 |
|-----|-----|------|
| **첫 번째** | 경도 (Longitude) | -180 ~ 180 |
| **두 번째** | 위도 (Latitude) | -90 ~ 90 |

**예시**:
```java
// ✅ 올바른 순서
Point seoul = new Point(126.9780, 37.5665);
                     // ────────  ───────
                     //   경도      위도

// ❌ 잘못된 순서 (위도, 경도)
Point wrong = new Point(37.5665, 126.9780);  // 오류 발생!
```

---

### 3.5.7.3 PharmacyRepository
...
public interface PharmacyRepository extends MongoRepository<Pharmacy, String> {
    ...
    // 반경 검색 (Near)
    List<Pharmacy> findByLocationNear(Point location, Distance distance);
...
**의미**: "주어진 중심점으로부터 지정된 거리 이내의 약국 찾기"

**MongoDB 연산자**: `$near` 또는 `$nearSphere`

**매개변수**:

| 매개변수 | 타입 | 역할 |
|---------|------|------|
| **`location`** | `Point` | 검색의 중심점 (내 위치) |
| **`distance`** | `Distance` | 검색할 반경 거리 |

**활용 사례**:
- "내 위치에서 5km 이내의 약국 찾기"
- "가장 가까운 ATM 찾기"
- "반경 3km 내의 맛집 찾기"
...
**의미**: "정의된 다각형 영역 내부에 있는 약국 찾기"

**MongoDB 연산자**: `$geoWithin`

**매개변수**:

| 매개변수 | 타입 | 역할 |
|---------|------|------|
| **`polygon`** | `Polygon` | 검색할 다각형 영역 |
...
public class PharmacyApplication implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) throws Exception {
        ...
        Point myLocation = new Point(
            126.9776214,  // 경도 (Longitude)
            37.5637072    // 위도 (Latitude)
        );
        
        Distance radius = new Distance(2.0, Metrics.KILOMETERS);
        
        List<Pharmacy> nearList = pharmacyRepository.findByLocationNear(myLocation, radius);
        
        log.info("--- 반경 2km 내 약국 ({}개) ---", nearList.size());
        for (Pharmacy pharmacy : nearList) {
        ...
        // 2. 영역 검색 (Within)
        // ========================================
        Polygon searchArea = new Polygon(
            new Point(126.734086, 37.751533),  // 좌상단
            new Point(127.269311, 37.751533),  // 우상단
            new Point(127.269311, 37.413294),  // 우하단
            new Point(126.734086, 37.413294),  // 좌하단
            new Point(126.734086, 37.751533)   // 시작점으로 돌아옴 (닫힘)
        );
        
        List<Pharmacy> withinList = pharmacyRepository.findByLocationWithin(searchArea);
        
        log.info("--- 지정 영역 내 약국 ({}개) ---", withinList.size());
        for (Pharmacy pharmacy : withinList) {
            log.info(pharmacy.toString());
        }
    }
}
...
### 3.5.7.6 중요한 Import 주의사항 ⚠️

**Spring Data Geo 타입 사용 (필수!)**

MongoDB 드라이버의 GeoJSON 타입이 아닌, **Spring Data Geo 프레임워크의 타입**을 사용해야 합니다!

| 클래스 | 사용 목적 | 패키지 (Import) |
|-------|----------|----------------|
| **`Point`** | 위치의 중심점 정의 | `org.springframework.data.geo.Point` |
| **`Polygon`** | 영역(Within)을 정의하는 다각형 | `org.springframework.data.geo.Polygon` |
| **`Distance`** | 검색의 반경 정의 (킬로미터 등) | `org.springframework.data.geo.Distance` |
| **`Metrics`** | 거리 단위 정의 (킬로미터, 마일 등) | `org.springframework.data.geo.Metrics` |

**❌ 잘못된 Import**:
```java
import com.mongodb.client.model.geojson.Point;  // ❌ MongoDB 드라이버 타입
import com.mongodb.client.model.geojson.Polygon;  // ❌
```

**✅ 올바른 Import**:
```java
import org.springframework.data.geo.Point;  // ✅ Spring Data Geo
import org.springframework.data.geo.Polygon;  // ✅
import org.springframework.data.geo.Distance;  // ✅
import org.springframework.data.geo.Metrics;  // ✅
```

**이유**: Spring Data MongoDB의 Repository 메서드(`findByLocationNear`, `findByLocationWithin`)는 **Spring Data Geo 타입**을 요구합니다.

---

### 3.5.7.7 Distance와 Metrics
...
| 매개변수 | 타입 | 역할 |
|---------|------|------|
| **첫 번째** | `double` | 거리 값 (2.0) |
| **두 번째** | `Metrics` | 거리 단위 (킬로미터, 마일 등) |

#### Metrics 종류

| 상수 | 단위 |
|-----|------|
| **`Metrics.KILOMETERS`** | 킬로미터 (km) |
| **`Metrics.MILES`** | 마일 (mile) |
| **`Metrics.NEUTRAL`** | 단위 없음 (도, degree) |
...
// 5마일 반경
new Distance(5.0, Metrics.MILES);

// 0.1도 반경 (약 11km)
new Distance(0.1, Metrics.NEUTRAL);
```

---

### 3.5.7.8 Polygon 생성 규칙

#### 다각형의 닫힘 (Closure)

**규칙**: 다각형의 **첫 번째 점과 마지막 점이 동일**해야 함 (닫힌 도형)

```java
Polygon searchArea = new Polygon(
    new Point(126.734086, 37.751533),  // 1. 좌상단
    new Point(127.269311, 37.751533),  // 2. 우상단
    new Point(127.269311, 37.413294),  // 3. 우하단
    new Point(126.734086, 37.413294),  // 4. 좌하단
    new Point(126.734086, 37.751533)   // 5. 시작점으로 돌아옴 (닫힘!)
);
```
...
**❌ 잘못된 예시**:
```java
// 마지막 점이 시작점과 다름 → 닫히지 않은 도형
Polygon wrong = new Polygon(
    new Point(126.734086, 37.751533),
    new Point(127.269311, 37.751533),
...
```
1. 중심점 설정
   Point myLocation = new Point(126.9776214, 37.5637072);
   ↓
2. 반경 설정
   Distance radius = new Distance(2.0, Metrics.KILOMETERS);
   ↓
3. Repository 메서드 호출
   List<Pharmacy> results = pharmacyRepository.findByLocationNear(myLocation, radius);
   ↓
4. MongoDB 쿼리 생성
   db.pharmacy.find({
     location: {
       $nearSphere: {
         $geometry: { type: "Point", coordinates: [126.9776214, 37.5637072] },
         $maxDistance: 2000  // 2km를 미터로 변환
       }
     }
   })
   ↓
5. 2DSPHERE 인덱스로 검색
   - location 필드의 공간 인덱스 사용
   - 지구 곡률 반영한 정확한 거리 계산
   ↓
6. 결과 반환
   [약국1(1.2km), 약국2(1.8km), 약국3(2.0km)]
```
...
```
1. 다각형 영역 정의
   Polygon area = new Polygon(점1, 점2, 점3, 점4, 점1);
   ↓
2. Repository 메서드 호출
   List<Pharmacy> results = pharmacyRepository.findByLocationWithin(area);
   ↓
3. MongoDB 쿼리 생성
   db.pharmacy.find({
     location: {
       $geoWithin: {
         $geometry: {
           type: "Polygon",
           coordinates: [[
             [126.734086, 37.751533],
             [127.269311, 37.751533],
             [127.269311, 37.413294],
   ...
   ↓
4. 영역 내부 판정
   - 각 약국의 location이 다각형 내부에 있는지 확인
   ↓
5. 결과 반환
   [영역 내 약국1, 영역 내 약국2, ...]
```
...
### 3.5.7.10 공간 검색 핵심 정리

#### MongoDB 공간 검색의 3요소

| 요소 | 설정 방법 | 역할 |
|-----|----------|------|
| **검색 대상 지정** | `@GeoSpatialIndexed` | 이 필드를 공간 검색 대상으로 지정 |
| **곡률 반영** | `GEO_2DSPHERE` | 지구 곡률을 반영한 정확한 거리 계산 |
| **데이터 타입** | `Point` | 위도/경도 좌표 저장 |
...
| 상황 | 키워드 | 매개변수 |
|-----|-------|---------|
| **반경 검색** | `Near` | `Point` (중심점) + `Distance` (반경) |
| **영역 검색** | `Within` | `Polygon` (다각형 영역) |

---

## 3.5.8 MongoDB vs RDBMS 비교 정리
...
|-----|-------|---------|
| **저장 단위** | 테이블 (Table) | 컬렉션 (Collection) |
| **데이터 단위** | 레코드/행 (Row) | 도큐먼트 (Document) |
| **데이터 속성** | 컬럼 (Column) | 필드 (Field) |
| **스키마** | 고정 (Strict) | **유연 (Schemaless)** ⭐ |
| **연관관계** | 외래 키 + JOIN | **비정규화 (De-Normalization)** ⭐ |

---

### 3.5.8.2 Spring Data 비교

| 항목 | Spring Data JPA | Spring Data MongoDB |
|-----|----------------|---------------------|
| **대상 DBMS** | RDBMS (MySQL, PostgreSQL 등) | NoSQL (MongoDB) |
| **클래스 매핑** | `@Entity` | **`@Document`** |
| **필드 매핑** | `@Column` | **`@Field`** |
| **기본 Repository** | `JpaRepository` | **`MongoRepository`** |
| **쿼리 메서드** | ✅ 지원 | ✅ **지원 (동일 규칙)** |
| **`@Query` 언어** | JPQL (객체 지향) | **JSON/BSON** (문서 지향) |
| **텍스트 검색** | 제한적 (`LIKE`) | **강력한 전문 검색** ⭐ |
| **공간 검색** | 제한적 | **강력한 지리 검색** ⭐ |

---

### 3.5.8.3 언제 MongoDB를 사용하는가?

#### MongoDB 사용 권장 사례 ⭐

| 상황 | 이유 |
|-----|------|
| **텍스트 검색이 중요한 경우** | 블로그, 뉴스, 쇼핑몰 등 (검색 점수, 가중치 지원) |
| **위치 기반 서비스** | 배달앱, 부동산, 지도 서비스 등 (공간 검색 강력) |
| **스키마가 자주 변경되는 경우** | 초기 개발, 프로토타입, 빠른 변화가 필요한 프로젝트 |
| **비정규화가 유리한 경우** | 읽기 성능이 중요하고, 데이터 중복 허용 가능 |
| **대용량 로그 저장** | 시스템 로그, 이벤트 기록 등 |
...
| 상황 | 이유 |
|-----|------|
| **복잡한 트랜잭션** | 은행 시스템, 회계 시스템 등 (ACID 보장 중요) |
| **정규화가 필수인 경우** | 데이터 중복 최소화, 일관성 중요 |
| **복잡한 JOIN이 많은 경우** | 관계형 데이터 모델링이 자연스러운 경우 |
| **스키마가 고정된 경우** | 안정적인 스키마, 엄격한 데이터 검증 필요 |

---

## 3.5.9 MongoDB 핵심 정리

### 3.5.9.1 MongoDB의 2대 핵심 기능

| 기능 | 핵심 어노테이션 | 주요 도구 | 활용 |
|-----|---------------|----------|------|
| **1. 텍스트 검색** | `@TextIndexed`, `@TextScore` | `TextCriteria` | 키워드 검색, 점수 기반 정렬 |
| **2. 공간 검색** | `@GeoSpatialIndexed` | `Point`, `Distance`, `Polygon` | 반경 검색, 영역 검색 |
...
| 어노테이션 | 사용 위치 | 역할 |
|-----------|---------|------|
| **`@Document`** | 클래스 | 컬렉션 매핑 (`@Entity`와 유사) |
| **`@Id`** | 필드 | 고유 식별자 (`_id`) |
| **`@Field`** | 필드 | 필드 매핑 (`@Column`과 유사) |
| **`@TextIndexed`** | 필드 | 텍스트 검색 대상 지정 + 가중치 설정 |
| **`@TextScore`** | 필드 | 검색 점수 저장 |
| **`@GeoSpatialIndexed`** | 필드 | 공간 검색 대상 지정 (지구 곡률 반영) |

---

### 3.5.9.3 Repository 메서드 패턴

#### 기본 CRUD (MongoRepository)
...
findByDescriptionStartingWith(String description)
```
...
```java
// TextCriteria 사용
findBy(TextCriteria criteria, Sort sort)
findByOrderByScoreDesc(TextCriteria criteria)  // 가장 많이 사용!
```
