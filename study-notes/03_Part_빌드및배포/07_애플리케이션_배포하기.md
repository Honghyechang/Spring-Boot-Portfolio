...
- [7.4 CI/CD 자동화 (GitHub Actions)](#74-cicd-자동화-github-actions)
---
...
**핵심 주제**: GitHub Actions를 활용한 완전 자동화 배포 파이프라인 구축  
**사용 기술**: GitHub Actions, Docker, AWS Elastic Beanstalk, YAML  
**학습 목표**: 
- CI/CD의 개념과 필요성을 실무 경험을 통해 체득
- GitHub Actions 워크플로우 작성 및 실행 원리 이해
- 수동 배포의 모든 단계를 자동화하여 생산성 극대화
- 코드 push 한 번으로 운영 환경까지 자동 배포되는 실무 환경 구축
...
7.3장까지 우리는 Docker를 활용한 다양한 배포 방식을 학습했습니다. 하지만 모든 배포 과정은 **수동**으로 진행되었습니다.
...
1. IntelliJ에서 코드 수정
   ↓
2. Terminal에서 gradle build 실행 (JAR 생성)
   ↓
3. docker build -t 이미지명 . (이미지 생성)
   ↓
4. docker tag ... (태그 변경)
   ↓
5. docker push ... (Docker Hub에 업로드)
   ↓
6. docker-compose.yml, nginx.conf, Dockerrun.aws.json을 ZIP으로 묶기
   ↓
7. AWS EB 콘솔 접속
   ↓
8. 파일 업로드 버튼 클릭
...
| 문제 유형 | 구체적 문제 | 영향 |
|----------|------------|------|
| **시간 소모** | 전체 과정 20분 이상 소요 | 개발 생산성 저하 |
| **휴먼 에러** | 단계를 빠뜨리거나 잘못된 명령 실행 | 배포 실패, 서비스 중단 위험 |
| **반복 작업** | 코드를 수정할 때마다 동일한 과정 반복 | 지루함, 실수 가능성 증가 |
| **협업 어려움** | 배포 과정을 모든 팀원이 숙지해야 함 | 팀 확장 시 교육 비용 증가 |

**실제 경험한 문제 사례**

```
상황: 게시판 제목을 "스프링 부트 게시판"에서 "SPRING BOOT BOARD"로 변경

문제 발생 시나리오:
1. 코드 수정 후 빌드는 했는데 docker build를 깜빡함
2. 이전 이미지를 push → 변경사항이 반영 안 됨
3. 30분 후 확인했을 때 여전히 구 버전 서비스 중
4. 다시 처음부터 전체 과정 반복...
```
...
**의미**: 코드를 자주 통합하고 자동으로 테스트하는 것

**동작 원리**

```
개발자 A: 로그인 기능 개발 → GitHub에 push
   ↓
GitHub Actions (CI 서버)가 자동으로:
   1. 두 사람의 코드를 병합(Merge)
   2. 빌드가 되는지 확인 (gradle build)
   3. 테스트 실행
   4. 문제 있으면 즉시 알림 발송
   ↓
개발자 B: 게시판 기능 개발 → GitHub에 push
   ↓
(동일한 자동화 과정 반복)
```

**핵심 가치**: 코드 통합 시 발생하는 충돌을 조기에 발견하고 해결

---

#### CD (Continuous Deployment/Delivery) - 지속적 배포

**의미**: 테스트를 통과한 코드를 자동으로 배포하는 것
...
**핵심 가치**: 배포 과정의 자동화로 휴먼 에러 제거 및 배포 속도 향상
...
| 항목 | 수동 배포 | CI/CD 자동화 |
|------|----------|-------------|
| **개발자 작업** | 빌드 → 이미지 생성 → 푸시 → ZIP 생성 → EB 업로드 | `git push` 한 번 |
| **소요 시간** | 20분 | 5분 (자동 진행) |
| **실수 가능성** | 높음 (8단계 수동) | 없음 (자동화) |
| **배포 빈도** | 주 1-2회 (번거로워서) | 하루 여러 번 (간편해서) |

---

## 7.4.3 GitHub Actions 소개

### 🤖 GitHub Actions란?

**정의**: GitHub에서 제공하는 무료 CI/CD 플랫폼

**비유로 이해하기**

```
당신: "야 GitHub야, 내가 코드 push하면 자동으로 이것저것 해줘"
GitHub Actions: "오케이! 뭘 해줄까?"
당신: "빌드하고, 이미지 만들고, 배포해줘" (YAML 파일에 작성)
GitHub Actions: "알았어! 자동으로 해줄게"
```
...
**의미**: 자동화할 작업의 전체 흐름을 정의한 설정 파일

**파일 형식**: YAML (`.yml` 또는 `.yaml`)

**저장 위치**: 저장소 루트의 `.github/workflows/` 디렉터리
...
**의미**: 워크플로우를 시작하게 만드는 트리거

**주요 이벤트 종류**

| 이벤트 | 실행 시점 | 주요 용도 |
|--------|----------|----------|
| `push` | 특정 브랜치에 코드를 push했을 때 | 배포 자동화 |
| `pull_request` | PR을 생성하거나 업데이트했을 때 | 코드 리뷰 전 자동 테스트 |
| `schedule` | 지정된 시간표(Cron)에 따라 | 정기 백업, 주기적 테스트 |
| `workflow_dispatch` | 수동으로 버튼 클릭 시 | 긴급 배포, 롤백 |
...
**의미**: 워크플로우 내에서 실행되는 독립적인 작업 단위

**핵심 원칙**: 각 Job은 독립된 가상 컴퓨터(Runner)에서 실행됩니다.
...
jobs:
  build:
    runs-on: ubuntu-latest  # 우분투 가상 컴퓨터 할당
  
  test:
    runs-on: ubuntu-latest  # 별도의 우분투 가상 컴퓨터 할당
```

**비유**: Job 하나 = 독립된 가상 컴퓨터 한 대

---

#### 4. Step (단계)

**의미**: Job 내에서 순차적으로 실행되는 개별 명령

**특징**: 

- **순차 실행**: 위에서 아래로 순서대로 실행
- **실패 시 중단**: 중간 단계가 실패하면 이후 단계는 실행되지 않음
...
| 타입 | 예시 | 설명 |
|------|------|------|
| **GitHub 공식** | `actions/checkout@v3` | GitHub이 제공하는 검증된 액션 |
| **써드파티** | `einaregilsson/beanstalk-deploy@v21` | 커뮤니티가 만든 액션 |
| **직접 명령** | `run: ./gradlew build` | 셸 명령어 직접 실행 |

**장점**: 복잡한 설치나 인증 절차를 한 줄로 해결

---

## 7.4.4 실습 준비: EB 환경 생성

CI/CD를 구축하기 전에, 배포 대상이 될 EB 환경을 먼저 생성해야 합니다.
...
| 항목 | 설정값 |
|------|--------|
| **애플리케이션 이름** | `hyechang-spring-cicd-app` |
| **환경 이름** | `Hyechang-spring-cicd-app` |
| **플랫폼** | Docker running on 64bit Amazon Linux 2023 |
| **환경 유형** | 단일 인스턴스 |

**코드 업로드**

앞서 7.3장에서 사용했던 파일 3개를 ZIP으로 묶어 업로드합니다.
...
**중요**: 이 파일들의 내용은 이전과 동일하지만, **처음 배포 시에는 환경 변수를 설정하지 않습니다.**

---

### ⚠️ DB 초기화 전략

**문제 상황**: RDS를 사용하지 않고 Docker MySQL 컨테이너를 사용하는 경우, `db-volume` 볼륨으로 인해 데이터가 영구 저장됩니다.

**초기화 충돌**

```
첫 배포: schema.sql 실행 → 테이블 생성 ✅
재배포: schema.sql 재실행 시도 → "Table 'member' already exists" 에러 ❌
```
...
| 단계 | 환경 변수 설정 | 목적 |
|------|--------------|------|
| **1차 배포** | 설정 없음 | 테이블 생성 허용 (`spring.sql.init.mode=always`) |
| **2차 배포 이후** | `SPRING_SQL_INIT_MODE=never` | 테이블 재생성 방지 |

**실제 적용 순서**

1. 환경 변수 없이 환경 생성 및 첫 배포
2. 배포 완료 후 EB 콘솔에서 환경 변수 추가
3. 이후 재배포 시 자동으로 충돌 방지

**주의**: 이 전략은 나중에 `docker-compose.yml`에 직접 명시하는 방식으로 개선됩니다.

---

### 🎯 환경 생성 완료

환경이 정상적으로 생성되고 애플리케이션이 실행되는 것을 확인합니다.

```
환경 URL: http://Hyechang-spring-cicd-app.eba-xxxxxxx.ap-northeast-2.elasticbeanstalk.com
상태: Ok ✅
```

---

## 7.4.5 AWS IAM 사용자 생성

GitHub Actions가 AWS 리소스에 접근하려면 **인증 자격 증명**이 필요합니다.

### 🔑 IAM 사용자의 역할

**핵심 개념**: IAM 사용자는 프로그래밍 방식으로 AWS에 접근할 수 있는 권한을 제공합니다.

**GitHub Actions의 작업 흐름**

```
GitHub Actions 서버
   ↓ (IAM 키 사용)
AWS S3 버킷에 deploy.zip 업로드
   ↓ (IAM 키 사용)
AWS EB에 배포 명령 전달
   ↓
EB가 S3에서 ZIP 다운로드 및 배포 수행
```
...
| 항목 | 설정값 |
|------|--------|
| **사용자 이름** | `github-actions-user` |
| **AWS Management Console 액세스** | 비활성화 (프로그래밍 방식 접근만 필요) |
...
| 정책 이름 | 권한 범위 | 필요한 이유 |
|----------|----------|-----------|
| `AmazonS3FullAccess` | S3 버킷 전체 제어 | `deploy.zip` 파일을 S3에 업로드하기 위함 |
| `AdministratorAccess-AWSElasticBeanstalk` | EB 환경 전체 제어 | EB에 배포 명령을 내리기 위함 |

**권한이 필요한 구체적인 작업**

```
GitHub Actions (IAM 키 사용)
   ↓
1. S3 업로드: deploy.zip을 S3 버킷에 저장
   → AmazonS3FullAccess 권한 필요
   ↓
2. EB 배포 요청: "이 S3 파일로 배포하라" 명령
   → AdministratorAccess-AWSElasticBeanstalk 권한 필요
```
...
```
IAM 콘솔 → 사용자 → github-actions-user → 보안 자격 증명 탭
```
...
```
설명: GitHub Actions CI/CD for Hyechang-spring-cicd-app
```
...
```
액세스 키 ID: 생성된 엑세스 키 ID
비밀 액세스 키: 생성된 비밀 엑세스 키
```
...
| 항목 | 역할 | 비유 |
|------|------|------|
| **액세스 키 ID** | 사용자 식별 | 사용자 이름 |
| **비밀 액세스 키** | 인증 | 비밀번호 |

**사용 예시**

```yaml
# deploy.yml에서 사용 (Secrets로 안전하게 관리)
...
## 7.4.6 GitHub Secrets 등록

생성한 AWS 키와 기타 민감한 정보를 GitHub 저장소의 Secrets에 안전하게 저장합니다.

### 🔒 GitHub Secrets란?

**의미**: GitHub 저장소에 암호화되어 저장되는 환경 변수

**보안 특징**

- ✅ 워크플로우 실행 시에만 복호화되어 사용
- ✅ GitHub Actions 로그에도 마스킹 처리되어 노출 안 됨
- ✅ 저장소 관리자만 확인 및 수정 가능
...
**New repository secret 버튼 클릭 후 다음 정보를 하나씩 등록합니다.**
...
| **Value** | `hyechang2071148` |

**용도**: Docker Hub 로그인 시 사용자명
...
#### Secret 6: EB_APPLICATION_NAME

| 항목 | 값 |
|------|-----|
| **Name** | `EB_APPLICATION_NAME` |
| **Value** | `hyechang-spring-cicd-app` |

**용도**: 배포 대상 EB 애플리케이션 이름

---

#### Secret 7: EB_ENVIRONMENT_NAME
...
|------|-----|
| **Name** | `S3_BUCKET_NAME` |
| **Value** | `my-eb-docker-creds-bucket-hyechang` |

**용도**: 

1. Docker 인증 파일(`.dockercfg`) 저장 위치
2. GitHub Actions가 `deploy.zip`을 임시 업로드할 위치
...
| 역할 | 파일 | 사용 주체 | 목적 |
|------|------|----------|------|
| **역할 1** | `.dockercfg` | EB 환경 | Docker Hub 인증 |
| **역할 2** | `deploy.zip` | GitHub Actions | 배포 패키지 전송 |

**결론**: 기존에 생성한 S3 버킷을 그대로 사용하면 됩니다.

---

### ✅ Secrets 등록 완료 확인

총 8개의 Secret이 등록되어 있어야 합니다.
...
GitHub Actions는 **저장소 루트의 `.github/workflows/` 경로**에 있는 YAML 파일을 자동으로 인식합니다.

**올바른 경로**

```
Spring-Boot-Portfolio/  ← 저장소 루트
├── .github/
│   └── workflows/
│       └── deploy.yml  ← 워크플로우 파일
├── spring-boot-project/
...
**⚠️ 주의**: 하위 프로젝트 폴더에 `.github`를 두면 인식되지 않습니다!
...
name: Deploy to AWS Elastic Beanstalk CI/CD

on:
  push:
    branches:
      - main  # main 브랜치에 push 할 때만 실행
...
jobs:
  deploy:
    ...
    steps:
      ...
      # 3단계: Gradle 빌드 (JAR 생성)
      - name: Build with Gradle
        run: ./gradlew clean bootJar
        working-directory: spring-boot-project/Spring-Board-Project
      ...
      - name: Build Docker image
        run: |
          docker build -t ${{ secrets.DOCKER_USERNAME }}/spring-compose-image:latest .
        working-directory: spring-boot-project/Spring-Board-Project
      
      # 6단계: Docker Hub에 푸시
      - name: Push Docker image
        run: |
          docker push ${{ secrets.DOCKER_USERNAME }}/spring-compose-image:latest
        working-directory: spring-boot-project/Spring-Board-Project
      
      # 7단계: 배포 ZIP 파일 생성
      - name: Create deployment package
        run: |
          zip -r deploy.zip docker-compose.yml nginx.conf Dockerrun.aws.json
        working-directory: spring-boot-project/Spring-Board-Project
      ...
      - name: Deploy to Elastic Beanstalk
        ...
        with:
          ...
          deployment_package: spring-boot-project/Spring-Board-Project/deploy.zip
```
...
**의미**: GitHub Actions 탭에 표시될 워크플로우의 이름
...
**의미**: "언제 이 워크플로우를 실행할 것인가?"

| 항목 | 설명 |
|------|------|
| `on: push` | Git push 이벤트 발생 시 |
...
| `jobs:` | 모든 작업의 시작점 |
| `deploy:` | 작업 이름 (사용자 지정) |
| `runs-on: ubuntu-latest` | 우분투 가상 컴퓨터에서 실행 |

**독립성**: 여러 Job을 정의하면 각각 독립된 가상 컴퓨터에서 병렬 실행됩니다.
...
**의미**: Job 내에서 순차적으로 실행할 명령 목록

**핵심 원칙**

- ✅ 위에서 아래로 순서대로 실행
- ✅ 중간 단계 실패 시 즉시 전체 Job 중단
- ✅ 이전 단계가 성공해야 다음 단계 실행
...
**의미**: 이 단계가 무엇을 하는지 설명하는 이름표

**로그 확인**: GitHub Actions 실행 로그에 이 이름으로 표시됩니다.
...
**의미**: 미리 만들어진 명령어 묶음(Action)을 가져와 사용
...
| 액션 | 역할 |
|------|------|
| `actions/checkout@v3` | 저장소 코드를 가상 컴퓨터로 복제 |
| `actions/setup-java@v3` | JDK 설치 및 환경 설정 |
| `docker/login-action@v2` | Docker Hub 로그인 |

**장점**: 복잡한 설치 과정을 한 줄로 해결
...
**파라미터 확인 방법**: GitHub Marketplace에서 액션 문서 확인
...
**의미**: 이 명령을 실행하기 전에 지정된 디렉터리로 이동

**필요한 이유**: 

```
저장소 루트
├── .github/workflows/deploy.yml  ← 워크플로우는 여기 있지만
└── spring-boot-project/
    └── Spring-Board-Project/
        └── gradlew  ← 실행할 파일은 여기 있음
```

**해결**: `working-directory`로 경로 지정
...
1. GitHub Actions가 실행 시점에 Secret 값을 불러옴
2. 워크플로우 내에서 해당 값으로 치환
3. 로그에는 `***`로 마스킹되어 표시

**주의**: Secret 변수명은 대소문자 구분!
...
우분투 가상 컴퓨터의 작업 디렉터리
   ↓
이후 단계에서 이 코드를 사용하여 빌드/배포
```

**복제 대상**: `.github/workflows/deploy.yml`을 포함한 모든 파일
...
**역할**: Gradle 빌드를 위한 Java 환경 준비

**필요성**: JAR 파일을 만들려면 JDK 필요

**자동 작업**

1. Eclipse Temurin JDK 21 다운로드
2. 가상 컴퓨터에 설치
...
**역할**: Spring Boot 실행 가능 JAR 파일 생성
...
| `clean` | 이전 빌드 결과물 삭제 |
| `bootJar` | Spring Boot JAR 파일 생성 |

**생성 위치**: `build/libs/Spring-Board-Project-0.0.1-SNAPSHOT.jar`
...
**역할**: Docker Hub 인증 (6단계 push를 위한 전제)

**동작**: Secret에 저장된 자격 증명으로 로그인
...
**역할**: 3단계에서 만든 JAR를 포함하는 Docker 이미지 생성

**명령 분석**

```bash
docker build -t hyechang2071148/spring-compose-image:latest .
```
...
| `-t` | 태그(이름) 지정 |
| `hyechang2071148/` | Docker Hub 사용자명 |
| `spring-compose-image` | 저장소 이름 |
| `latest` | 버전 태그 |
| `.` | 현재 디렉터리 (Dockerfile 위치) |

**Dockerfile 사용**: 현재 디렉터리의 `Dockerfile`을 읽어 이미지 빌드
...
**역할**: EB 배포에 필요한 설정 파일 3개를 ZIP으로 압축
...
└── Dockerrun.aws.json  ← Docker Hub 인증 정보
```

**중요**: JAR 파일은 포함되지 않음 (이미 Docker 이미지에 포함됨)
...
**역할**: ZIP 파일을 S3에 업로드하고 EB에 배포 명령 전달

**파라미터 상세**

| 파라미터 | 예시 값 | 설명 |
|----------|---------|------|
| `aws_access_key` | `AKIA...` | AWS 인증 - 사용자명 |
| `aws_secret_key` | `sft8...` | AWS 인증 - 비밀번호 |
| `application_name` | `hyechang-spring-cicd-app` | EB 애플리케이션 이름 |
| `environment_name` | `Hyechang-spring-cicd-app` | EB 환경 이름 |
| `version_label` | `${{ github.sha }}` | 커밋 고유 ID (버전 추적) |
| `region` | `ap-northeast-2` | 서울 리전 |
| `deployment_package` | `spring-boot.../deploy.zip` | ZIP 파일 경로 |
...
1. IAM 키로 AWS 인증
   ↓
2. deploy.zip을 S3 버킷에 업로드
   ↓
3. EB API 호출: "이 ZIP으로 배포하라"
   ↓
4. EB가 S3에서 ZIP 다운로드
   ↓
5. docker-compose.yml 읽고 Docker Hub에서 이미지 다운로드
   ↓
6. 컨테이너 재시작 및 배포 완료
```
...
```
3단계: ./gradlew clean bootJar 실행 시
Permission denied 에러 발생
```

**원인**: Windows에서 작업한 `gradlew` 파일에 Linux 실행 권한이 없음
...
- Git은 파일 내용뿐 아니라 **메타데이터**(실행 권한 등)도 관리
- Windows는 실행 권한 개념이 없어 Git에 명시적으로 알려줘야 함
- 이후 Linux 환경(GitHub Actions)에서 실행 가능

---

### 📂 프로젝트 구조 문제

**문제 발생**: 워크플로우가 인식되지 않음

**원인**: `.github` 폴더가 하위 프로젝트에 있었음
...
**추가 설정**: `working-directory` 지정

```yaml
# 프로젝트가 하위 폴더에 있으므로 경로 지정
working-directory: spring-boot-project/Spring-Board-Project
```
...
# 2. .github 폴더를 루트로 이동 (수동)

# 3. deploy.yml에 working-directory 추가 (수동)

# 4. 변경사항 커밋
git add .
git commit -m "fix: Apply working-directory settings and update deploy.yml"
...
```
에러: Table 'member' already exists
```

**원인 분석**

```
docker-compose.yml의 db-volume:
   ↓
MySQL 데이터가 영구 저장됨
   ↓
재배포 시 schema.sql 재실행 시도
   ↓
이미 존재하는 테이블 생성 시도
   ↓
충돌!
...
| 우선순위 | 설정 위치 | 값 | 결과 |
|----------|----------|-----|------|
| 1 | `docker-compose.yml` | (없음) | - |
| 2 | `application-docker.properties` | `always` | ❌ 테이블 재생성 시도 |
| 3 | EB 환경 변수 | `never` | ⚠️ 무시됨 |

**해결책**: `docker-compose.yml`에 직접 명시
...
services:
  app:
    environment:
      ...
      DB_PASSWORD: password
      SPRING_SQL_INIT_MODE: never                    # 추가!
      SPRING_SESSION_JDBC_INITIALIZE_SCHEMA: never  # 추가!
```
...
git add .
git commit -m "fix: Set DB init modes to never in docker-compose environment section"
git push origin main
```
...
## 7.4.10 실전 테스트: 코드 변경 자동 배포

이제 CI/CD 파이프라인이 완벽하게 작동하는지 실제 코드 변경으로 테스트해봅시다.
...
git status
git add .
git commit -m "feat: Update site title to SPRING BOOT BOARD"
git push origin main
```
...
```
워크플로우 실행 중:
✓ feat: Update site title to SPRING BOOT BOARD #4
  
실행 단계:
✓ 1단계: Checkout code
✓ 2단계: Set up JDK 21
✓ 3단계: Build with Gradle
✓ 4단계: Log in to Docker Hub
✓ 5단계: Build Docker image
✓ 6단계: Push Docker image
✓ 7단계: Create deployment package
⏳ 8단계: Deploy to Elastic Beanstalk (진행 중...)
```
...
```
환경: Hyechang-spring-cicd-app
최근 이벤트: 환경 업데이트 완료
버전: feat_Update_site_title_to_SPRING_BOOT_BOARD-6984a92
상태: Ok ✅
```
...
```
http://Hyechang-spring-cicd-app.eba-xxxxxxx.ap-northeast-2.elasticbeanstalk.com

게시판 제목: SPRING BOOT BOARD ✅
```
...
```
1. base-layout.html 파일 수정
2. git push origin main
```

**GitHub Actions가 자동으로 수행한 작업**
...
7. S3에 업로드
8. EB에 배포 명령
9. EB가 새 이미지로 컨테이너 재시작
10. 배포 완료!
```

---

## 7.4.11 수동 배포 vs 자동 배포 최종 비교

### 📊 프로세스 비교

| 단계 | 수동 배포 (이전) | CI/CD 자동화 (현재) |
|------|-----------------|-------------------|
| **코드 수정** | IntelliJ에서 수정 | IntelliJ에서 수정 |
| **빌드** | `./gradlew build` 실행 | 자동 실행 |
| **이미지 생성** | `docker build` 명령 | 자동 실행 |
| **태그 변경** | `docker tag` 명령 | 자동 실행 |
| **Hub 푸시** | `docker push` 명령 | 자동 실행 |
| **ZIP 생성** | 수동으로 파일 압축 | 자동 실행 |
| **EB 업로드** | 콘솔 접속 및 업로드 | 자동 실행 |
| **배포 확인** | 수동 확인 | 자동 실행 |
...
|------|----------|-------------|
| **개발자 작업 시간** | 20분 | 30초 |
| **전체 배포 시간** | 20분 | 5분 (백그라운드) |
| **에러 발생 가능성** | 높음 (8단계 수동) | 없음 (자동화) |
| **스트레스** | 높음 | 없음 |
...
```
개발자: "배포해야 하는데... 20분이나 걸리니까 내일 한꺼번에 하자"
결과: 배포 빈도 ↓, 피드백 주기 ↑, 버그 누적
```

#### After CI/CD

```
개발자: "코드 수정했으니 바로 push!"
결과: 배포 빈도 ↑, 피드백 주기 ↓, 빠른 버그 수정
```

---

## 7.4.12 CI/CD 파이프라인 최종 흐름도
...
└────────────────────┬────────────────────────────────────────┘
                     ...
                     ▼
┌─────────────────────────────────────────────────────────────┐
│               AWS Elastic Beanstalk (자동 처리)             │
│                                                             │
│  1. S3에서 deploy.zip 다운로드                              │
│  2. docker-compose.yml 읽기                                 │
│  3. Docker Hub에서 새 이미지 다운로드                        │
│  4. 기존 컨테이너 중지                                       │
│  5. 새 이미지로 컨테이너 시작                                │
│  6. 헬스 체크 통과 확인                                      │
│  7. 배포 완료!                                               │
...
**증상**: GitHub Actions 탭에 아무것도 표시 안 됨
...
```bash
# .github 폴더를 저장소 루트로 이동
mv spring-boot-project/Spring-Board-Project/.github ./
```
...
2. `DOCKER_USERNAME`, `DOCKER_PASSWORD` 정확한지 확인
3. 대소문자 구분 주의
...
**증상**: `Table 'member' already exists`

**원인**: `spring.sql.init.mode=always` 설정
...
#### 문제 5: 배포 ZIP 파일을 찾을 수 없음

**증상**: `Error: Deployment package not found`

**원인**: `deployment_package` 경로 오류
...
## 7.4.15 학습 정리 및 실무 적용

### ✅ 학습 완료 내용

| 번호 | 주제 | 핵심 내용 |
|------|------|----------|
| 1 | **CI/CD 개념** | 지속적 통합 및 배포의 필요성과 가치 이해 |
| 2 | **GitHub Actions** | 워크플로우 작성 및 실행 원리 체득 |
| 3 | **IAM 관리** | AWS 권한 관리 및 프로그래밍 방식 인증 |
| 4 | **Secrets 관리** | 민감한 정보의 안전한 저장 및 사용 |
| 5 | **자동화 구축** | 8단계 수동 배포를 완전 자동화 |
| 6 | **문제 해결** | 실제 발생한 문제들을 스스로 해결 |
...
- ✅ **YAML 작성**: 워크플로우 파일 구조 이해 및 작성
- ✅ **Git 활용**: 브랜치 전략 및 커밋 메시지 관리
- ✅ **Docker 통합**: 이미지 빌드-푸시 자동화
- ✅ **AWS 연동**: IAM, S3, EB 서비스 통합

**실무 경험**

- ✅ **생산성 향상**: 배포 시간 20분 → 5분 단축
- ✅ **품질 향상**: 휴먼 에러 제거, 안정적인 배포
- ✅ **DevOps 실전**: 현대적인 개발 프랙티스 체험
...
```
1. 실무 CI/CD 파이프라인 이해
2. Jenkins, GitLab CI 등 다른 도구로 확장 가능
3. Kubernetes, ECS 등 고급 배포 전략 학습 기반
```
